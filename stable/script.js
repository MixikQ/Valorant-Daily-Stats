...
const RankPictureObj = document.getElementsByClassName(`RankPicture`)[0];
const CurrentRankObj = document.getElementsByClassName(`CurrentRank`)[0];
const DrawsObj = document.getElementsByClassName(`Draws`)[0];
const WinsObj = document.getElementsByClassName(`Wins`)[0];
const LossesObj = document.getElementsByClassName(`Losses`)[0];
const EloChangesTextObj = document.getElementsByClassName(`EloChangesText`)[0];
const PlusObj = document.getElementsByClassName(`Plus`)[0];
const MinusObj = document.getElementsByClassName(`Minus`)[0];
const CurrentEloTextObj = document.getElementsByClassName(`CurrentEloText`)[0];
const ProgressBarProgressObj = document.getElementsByClassName(`ProgressBarProgress`)[0];
...
async function getNeededElo() {
  try {
    ...
    console.log(json);
    if (
      json.data.players[499] &&
      json.data.players[499].rr > json.data.thresholds[3].threshold
    ) {
...
async function checkGame(gameId) {
  try {
    ...
    for (i = 0; i < 10; ++i) {
      if (
        json.data.players[i].name.toLowerCase() == nickname.toLowerCase() &&
        json.data.players[i].tag.toLowerCase() == tag.toLowerCase()
      ) {
    ...
    for (i = 0; i < 2; ++i) {
      if (json.data.teams[i].team_id == playerTeam) {
        if (json.data.teams[i].won == true) {
          ...
          json.data.teams[i].rounds.won == json.data.teams[i].rounds.lost
        ) {
          ++Draws;
          return;
        } else {
...
async function setPageData() {
  try {
    RankPictureObj.src = `https://media.valorant-api.com/competitivetiers/03621f52-342b-cf4e-4f86-9350a49c6d04/${CurrentTier}/largeicon.png`;
    if (CurrentTier != 0) {
      if (CurrentPlace) {
        CurrentRankObj.innerHTML = `${CurrentRank} #${CurrentPlace}`;
      } else {
        CurrentRankObj.innerHTML = `${CurrentRank}`;
      }
    } else {
      if (GamesForRating == 1) {
        CurrentRankObj.innerHTML = `${GamesForRating} game to rank`;
      } else {
        CurrentRankObj.innerHTML = `${GamesForRating} games to rank`;
      }
    }
    CurrentRankObj.style.setProperty(`color`, text_color);
    DrawsObj.innerHTML =
      Draws >= 10 ? `Draws: ${Draws}` : `Draws: <i>0</i>${Draws}`;
    if (Draws > 0) {
      DrawsObj.style.setProperty(`color`, text_color);
    }
    WinsObj.innerHTML = Wins >= 10 ? `Wins: ${Wins}` : `Wins: <i>0</i>${Wins}`;
    WinsObj.style.setProperty(`color`, text_color);
    LossesObj.innerHTML =
      Losses >= 10 ? `Losses: ${Losses}` : `Losses: <i>0</i>${Losses}`;
    LossesObj.style.setProperty(`color`, text_color);
    if (Wins > 0 || Losses > 0 || Draws > 0) {
      EloChangesTextObj.style.color = `${text_color}`;
      if (EloChanges == 0) {
        EloChangesTextObj.innerHTML = `${EloChanges}<i>00</i>`;
      }
    }
    if (EloChanges > 0) {
      if (EloChanges >= 100) {
        EloChangesTextObj.innerHTML = `${EloChanges}`;
      } else if (EloChanges >= 10) {
        EloChangesTextObj.innerHTML = `${EloChanges}<i>0</i>`;
      } else {
        EloChangesTextObj.innerHTML = `${EloChanges}<i>00</i>`;
      }
      EloChangesTextObj.style.setProperty(`letter-spacing`, `0px`);
      PlusObj.style.setProperty(`color`, text_color);
      MinusObj.style.setProperty(`background-color`, `#00000000`);
      ...
      EloChangesTextObj.style.setProperty(`letter-spacing`, `0px`);
      PlusObj.style.setProperty(`color`, `#00000020`);
      MinusObj.style.setProperty(`background-color`, text_color);
    } else {
      EloChangesTextObj.style.setProperty(`letter-spacing`, `-2px`);
      PlusObj.style.setProperty(`color`, `#00000020`);
      MinusObj.style.setProperty(`background-color`, `#00000000`);
    }
    if (CurrentTier != 0) {
      ...
      CurrentEloTextObj.innerHTML = `${CurrentEloString}/${NeededEloString}`;
    } else {
      CurrentEloTextObj.innerHTML = `Calibration`;
    }
    CurrentEloTextObj.style.setProperty(`color`, text_color);
    ProgressBarProgressObj.style.transform =
      CurrentTier >= 24
        ? `translateX(${Math.min(-50 + (CurrentElo / NeededElo) * 50, 0)}%)`
        : `translateX(${Math.min(-50 + CurrentElo / 2, 0)}%)`;
  } catch (error) {
